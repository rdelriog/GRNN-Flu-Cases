...
def MultipleModels(modelsDict, data, ind_train, ind_val, ind_test, nEpochs, batchSize, seqLen, stateFeat,
                   rnnStateFeat,  **kwargs):
    """
    ...
    for epoch in range(nEpochs):
        ...
        for batch in range(nBatches):
            ...
            for key in modelsDict.keys():
                ...
                # Set the ordering
                #xTrain = xTrain.reshape(nBatches, 1, nTrain*K)
                ...
                # Compute loss
                lossValueTrain = modelsDict[key].loss(yHatTrain[:,:,:,ind_train], yTrainModel[:,:,:,ind_train])
                ...
                accTrain = data.evaluate(yHatTrain.data[:,:,:,ind_train], yTrainModel[:,:,:,ind_train])
                ...
                for key in modelsDict.keys():
                    ...
                    with torch.no_grad():
                       ...
                       # Compute loss
                        ...
                        # Compute accuracy:
                        accValid = data.evaluate(yHatValid[:,:,:,ind_val], yValidModel[:,:,:,ind_val])
