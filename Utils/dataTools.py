...
class KStepPrediction():
    ...
    def __init__(self, K, G, nTrain, nValid, nTest, horizon, sigmaSpatial = 1, sigmaTemporal = 0,
                 rhoSpatial = 0, rhoTemporal = 0, alpha = None, dataType = np.float64, device = 'cpu'):
        # store attributes
        self.dataType = dataType
        self.device = device
        self.K = K
        ...
        for t in range(self.horizon):
            ...
            if alpha <= 1:
                x_tplus1 = alpha*x_t + spatialNoise + tempNoise[t, :, :]
                #print(x_t.shape)
            elif alpha > 1:
                alpha -= 1
                #print(x_t.shape)
                x_tplus1 = np.matmul(x_t,scipy.linalg.fractional_matrix_power(A, alpha)) 
                + spatialNoise + tempNoise[t, :, :]
            else:
...
class FluPrediction():
    ...
    def __init__(self, K, G, nTotal, nTrain, nValid, nTest, horizon, sigmaSpatial = 1, sigmaTemporal = 0,
                 rhoSpatial = 0, rhoTemporal = 0, alpha = None, dataType = np.float64, device = 'cpu'):
        # store attributes
        self.dataType = dataType
        self.device = device
        ...
        x_t = np.zeros((nTotal, nWeeks, nFeatures, nNodes));
        y_t = np.zeros((nTotal, nWeeks, nFeatures, nNodes));
        ...
        for b in range(nTotal):
            for w in range(nWeeks):
                for n in range(nFeatures):
                    line_count = 0
                    flu_NY = csv.reader(open("FluDataNY.csv","rt"))
                    for row in flu_NY:
                        if line_count <= 1:
                            line_count += 1
                        else:
                            county = int((int(str(row[1])[2:])+1)/2 - 1)
                            x_t[b][w][n][county] = row[w + 2]
                            y_t[b][w][n][county] = row[w + 2 + nWeeks]

        x = torch.from_numpy(x_t)
        y = torch.from_numpy(y_t)
        
                
        y = y.reshape(nTotal, -1)
        x = x.reshape(nTotal, -1)
        # Now, we have the signals and the labels
        signals = x # nTotal x N (CS notation)
        labels = y
        # Split and save them
        self.samples = {}
        self.samples['train'] = {}
        self.samples['train']['signals'] = signals[:nTrain, :].double()
        #self.samples['train']['signals'] = self.samples['train']['signals'].reshape(1, -1)
        self.samples['train']['labels'] = labels[:nTrain, :].double()
        #self.samples['train']['labels'] = self.samples['train']['labels'].reshape(1, -1)
        self.samples['valid'] = {} 
        self.samples['valid']['signals'] = signals[:nValid, :].double()
        #self.samples['valid']['signals'] = self.samples['valid']['signals'].reshape(1, -1)
        self.samples['valid']['labels'] = labels[:nValid, :].double()
        #self.samples['valid']['labels'] = self.samples['valid']['labels'].reshape(1, -1)
        self.samples['test'] = {}
        self.samples['test']['signals'] = signals[:nTest, :].double()
        #self.samples['test']['signals'] = self.samples['test']['signals'].reshape(1, -1)
        self.samples['test']['labels'] = labels[:nTest, :].double()
        #self.samples['test']['labels'] = self.samples['test']['labels'].reshape(1, -1)
        # Change data to specified type and device
        self.astype(self.dataType)
        self.to(self.device)
